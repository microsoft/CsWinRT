...
| CsWinRTPackWinMDInputs | *true \| false | When generating a reference projection, adds `CsWinRTInputs` to the NuGet package under `metadata/`, so consumers can pass them back as `CsWinRTInputs` to generate the merged projection. Set to `false` to lay out the package manually |
