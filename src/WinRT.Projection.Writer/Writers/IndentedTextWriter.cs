...
internal sealed partial class IndentedTextWriter
{
    ...
    public void Write(bool isMultiline, ReadOnlySpan<char> content)
    {
        ...
        // Add a newline before the content if we're mid-line after a brace and the new content starts with whitespace,
        // to prevent jamming the new content onto the same line as the brace. This is a common pattern when emitting
        // indented code statements after a multi-line raw string that ends with a brace (since raw `"""..."""` literals
        // never include a trailing newline before the closing token).
        if (_buffer.Length > 0 &&
            _buffer[^1] is '{' or '}' &&
            (isMultiline || content[0] is ' ' or '\t'))
        {
