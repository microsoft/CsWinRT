...
internal partial class ProjectionGenerator
{
    ...
    private static void EmitWinRTComponentSources(ProjectionGeneratorArgs args, ProjectionGeneratorProcessingState processingState)
    {
        ...
        WriteComponentActivationEntryPoints(processingState);
        WriteMergedManagedExports(processingState);
    }

    /// <summary>
    /// Emits an empty <c>ABI.{ComponentName}.ManagedExports</c> for every component assembly, for the case
    /// where the projection writer did not run at all.
    /// </summary>
    /// <remarks>
    /// A component whose types are all implementations of runtime classes declared in existing metadata
    /// contributes no types to project, so the writer (which normally emits these types, one per component
    /// with activatable classes) is skipped entirely. The component still forwards activation here
    /// unconditionally, so the entry point has to exist for the forward to resolve and report "not
    /// activatable here" rather than failing.
    /// </remarks>
    private static void WriteComponentActivationEntryPoints(ProjectionGeneratorProcessingState processingState)
    {
        ...
        foreach (string componentAssemblyName in processingState.ComponentAssemblyNames)
        {
            ...
            _ = builder.AppendLine($$"""
                ...
                namespace ABI.{{EscapeIdentifierName(componentAssemblyName)}};

                /// <summary>
                /// Managed activation entry point for the '{{componentAssemblyName}}' Windows Runtime component.
                /// </summary>
                [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
                public static unsafe class ManagedExports
                {
                    /// <summary>
                    /// Retrieves the activation factory for an activatable Windows Runtime class.
                    /// </summary>
                    /// <param name="activatableClassId">The class identifier that is associated with an activatable runtime class.</param>
                    /// <returns>Always <see langword="null"/>: this component declares no activatable Windows Runtime classes.</returns>
            ...
            File.WriteAllText(
                Path.Combine(processingState.SourcesFolder, $"{componentAssemblyName}.ManagedExports.g.cs"),
                builder.ToString());
        }
    }
