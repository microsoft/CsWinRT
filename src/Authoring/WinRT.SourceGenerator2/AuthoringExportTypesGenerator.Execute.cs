...
public partial class AuthoringExportTypesGenerator
{
    ...
    private static class Execute
    {
        ...
        public static void EmitManagedExports(SourceProductionContext context, AuthoringManagedExportsInfo info)
        {
            ...
            using (writer.WriteBlock())
            {
                ...
                using (writer.WriteBlock())
                {
                    ...
                    {
                        writer.WriteLine($"""
                            [UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "GetActivationFactory")]
                            static extern void* AuthoringGetActivationFactory(
                                [UnsafeAccessorType("ABI.{info.AssemblyName.EscapeIdentifierName()}.ManagedExports, WinRT.Component")] object? _,
        ...
        /// <summary>
        /// Emits an idempotent <c>[ModuleInitializer]</c>-driven <c>SetEntryAssembly</c> for
        /// component dlls. In hosted scenarios (e.g. <c>WinRT.Host.dll</c> activating the
        /// component) the .NET runtime has no natural entry assembly, and TypeMap discovery
        /// requires one. This emission designates the component dll as the entry assembly
        /// only when one isn't already set, so that in multi-component aggregator scenarios
        /// where <c>WinRT.Component.dll</c> sets itself first, this initializer is a no-op.
        /// </summary>
        /// <param name="context">The <see cref="SourceProductionContext"/> instance to use.</param>
        /// <param name="info">The input info.</param>
        public static void EmitProjectionTypesInitializer(SourceProductionContext context, AuthoringNativeExportsInfo info)
        {
            // Only emit for components. In merge-only / non-component scenarios there's
            // typically a managed exe driving the load and the entry assembly is already set.
            if (!info.Options.IsComponent)
            {
                return;
            }
            ...
            string source = $$"""
                ...
                /// <summary>
                /// Designates the component dll as the entry assembly for the hosted .NET runtime
                /// so that <c>[TypeMapAssemblyTarget]</c> discovery (rooted at the entry assembly)
                /// works. Idempotent: skips if an entry assembly is already set (e.g. when this
                /// component is loaded as a participant in a multi-component aggregation and
                /// <c>WinRT.Component.dll</c> set itself first).
                /// </summary>
            ...
            context.AddSource("ProjectionTypesInitializer.g.cs", source);
        }
    }
}