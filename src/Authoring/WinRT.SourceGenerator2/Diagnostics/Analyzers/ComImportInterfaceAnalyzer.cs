...
{
    /// <inheritdoc/>
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = [DiagnosticDescriptors.ComImportInterfaceCast];
        ...
        context.RegisterOperationAction(static context =>
        {
            if (context.Operation is IConversionOperation { Type: INamedTypeSymbol { TypeKind: TypeKind.Interface, IsComImport: true } interfaceType })
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    DiagnosticDescriptors.ComImportInterfaceCast,
