...
namespace Generator
{
    ...
    public class WinRTAotSourceGenerator : IIncrementalGenerator
    {
        ...
        private static (VtableAttribute, EquatableArray<VtableAttribute>) GetVtableAttributeToAdd(GeneratorSyntaxContext context, TypeMapper typeMapper)
        {
            var symbol = context.SemanticModel.GetDeclaredSymbol(context.Node as ClassDeclarationSyntax);
            var vtableAttribute = GetVtableAttributeToAdd(symbol, GeneratorHelper.IsWinRTType, typeMapper, context.SemanticModel.Compilation, false);
            if (vtableAttribute != default)
            {
                ...
                AddVtableAdapterTypeForKnownInterface(symbol, context.SemanticModel.Compilation, GeneratorHelper.IsWinRTType, typeMapper, vtableAttributesForLookupTable);
        ...
        private static (VtableAttribute, VtableAttribute) GetVtableAttributesForTaskAdapters(GeneratorSyntaxContext context, TypeMapper mapper)
        {
                ...
                if (!string.IsNullOrEmpty(symbolStr))
                {
                    if (AsyncMethodToTaskAdapter.TryGetValue(symbolStr, out var adapterTypeStr))
                    {
                        ...
                        if (adpaterType is not null)
                        {
                            ...
                            return (GetVtableAttributeToAdd(constructedAdapterType, GeneratorHelper.IsWinRTType, mapper, context.SemanticModel.Compilation, false), 
                                    GetVtableAttributeToAdd(constructedAdapterType, IsWinRTTypeWithPotentialAuthoringComponentTypes, mapper, context.SemanticModel.Compilation, false));
                        }
                    }
                }
            }
            ...
            bool IsWinRTTypeWithPotentialAuthoringComponentTypes(ISymbol type, TypeMapper mapper)
            {
                var winrtTypeAttribute = context.SemanticModel.Compilation.GetTypeByMetadataName("WinRT.WindowsRuntimeTypeAttribute");
                return GeneratorHelper.IsWinRTType(type, winrtTypeAttribute, mapper, true, context.SemanticModel.Compilation.Assembly);
            }
        }
        ...
        private static EquatableArray<VtableAttribute> GetVtableAttributesToAddOnLookupTable(GeneratorSyntaxContext context, TypeMapper mapper)
        {
            ...
            if (context.Node is InvocationExpressionSyntax invocation)
            {
                ...
                if (invocationSymbol is IMethodSymbol methodSymbol &&
                    ...
                    (GeneratorHelper.IsWinRTType(methodSymbol.ContainingSymbol, mapper) ||
            ...
            else if (context.Node is AssignmentExpressionSyntax assignment)
            {
                     ...
                     SymbolEqualityComparer.Default.Equals(propertySymbol.ContainingAssembly, context.SemanticModel.Compilation.Assembly)))
                {
                    ...
                    (GeneratorHelper.IsWinRTType(fieldSymbol.ContainingSymbol, mapper) || 
            ...
            void AddVtableAttributesForType(Microsoft.CodeAnalysis.TypeInfo instantiatedType, ITypeSymbol convertedToTypeSymbol)
            {
                ...
                if (instantiatedType.Type is IArrayTypeSymbol arrayType)
                {
                    if (convertedToTypeSymbol is not IArrayTypeSymbol)
                    {
                        ...
                        var vtableAtribute = GetVtableAttributeToAdd(arrayType, GeneratorHelper.IsWinRTType, mapper, context.SemanticModel.Compilation, false);
                ...
                else if (instantiatedType.Type is not null || instantiatedType.ConvertedType is not null)
                {
                    ...
                    if (instantiatedTypeSymbol.TypeKind == TypeKind.Delegate &&
                        ...
                        GeneratorHelper.IsWinRTType(instantiatedTypeSymbol, mapper) &&
                        convertedToTypeSymbol.SpecialType == SpecialType.System_Object)
                    {
                        ...
                        vtableAttributes.Add(GetVtableAttributeToAdd(instantiatedTypeSymbol, GeneratorHelper.IsWinRTType, mapper, context.SemanticModel.Compilation, false));
                    }
                    ...
                    if (instantiatedTypeSymbol.TypeKind == TypeKind.Class &&
                        (instantiatedTypeSymbol.MetadataName.Contains("`") || !GeneratorHelper.IsWinRTType(instantiatedTypeSymbol, mapper)) &&
                    ...
                    {
                        var vtableAtribute = GetVtableAttributeToAdd(instantiatedTypeSymbol, GeneratorHelper.IsWinRTType, mapper, context.SemanticModel.Compilation, false);
                        if (vtableAtribute != default)
                        {
                            vtableAttributes.Add(vtableAtribute);
                        }

                        AddVtableAdapterTypeForKnownInterface(instantiatedTypeSymbol, context.SemanticModel.Compilation, GeneratorHelper.IsWinRTType, mapper, vtableAttributes);
                    }
                }
            }
        }
