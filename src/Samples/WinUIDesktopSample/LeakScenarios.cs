...
namespace WinUIDesktopSample
{
    ...
    public class LeakScenarios
    {
        ...
        private WeakReference withoutCapture;
        public void WithoutCapture_Click(Action<string> report)
        {
            report("Grid and handler without capture");

            // Succeeds, as there's no cycle between object and event handler
            withoutCapture = CreateObject(withCapture: false);
        }
        public void WithoutCapture_Check(Action<string> report)
        {
            report("Check grid and handler without capture");
            GC.Collect(2, GCCollectionMode.Forced, true);
            GC.WaitForPendingFinalizers();
            report(withoutCapture.IsAlive ? "Grid leaked" : "Grid collected");
        }

        private WeakReference withCapture;
        public void WithCapture_Click(Action<string> report)
        {
            report("Grid and handler with capture");
            // Fails due to cycle between object and event handler (unlike UWP)
            withCapture = CreateObject(withCapture: true);
        }
        public void WithCapture_Check(Action<string> report)
        {
            report("Check grid and handler with capture");
            GC.Collect(2, GCCollectionMode.Forced, true);
            GC.WaitForPendingFinalizers();
            report(withCapture.IsAlive ? "Grid leaked" : "Grid collected");
        }

        public void Alloc_Click(Action<string> report)
        {
            report("Allocate various aggregates");
            ...
            //var ah = grid.ActualHeight;  TODO: does this also leak?
            ...
            report("(click Check Leaks repeatedly)");
        }

        public void Check_Click(Action<string> report)
        {
            report("Check for leaks of aggregates");
            ...
            report(baseStatus + ", " + derivedStatus + ", " + gridStatus + ", " + derivedGridStatus);
        }
    }
    ...
    public class ARRPage : ICustomQueryInterface
    {
        ...
        {
            return LeakScenarios.CreatePage(outer, out inner);
        }
