    ...
    where T : MulticastDelegate
{
    ...
    /// <summary>
    /// Map of all registered <typeparamref name="T"/> handlers to their registration tokens.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The value type is just <see cref="object"/> to reduce the generic instantiations.
    /// </para>
    /// </remarks>
    private readonly Dictionary<int, object> m_tokens = [];

    /// <summary>
    /// The current counter used for the low 32 bits of the registration tokens.
    /// </summary>
    /// <remarks>
    /// We explicitly use <c>[<see cref="int.MinValue"/>, <see cref="int.MaxValue"/>]</c> as the
    /// range, as this value is expected to eventually wrap around, and we don't want to lose the
    /// additional possible range of negative values (there's no reason for that).
    /// </remarks>
    private int m_low32Bits = Random.Shared.Next(int.MinValue, int.MaxValue);
    ...
    public bool RemoveEventHandler(EventRegistrationToken token, [NotNullWhen(true)] out T? handler)
    {
        ...
        lock (m_tokens)
        {
            // Use a single lookup to both check whether the token exists in the
            // table, remove it, and also retrieve the removed handler to return.
    ...
    /// <summary>
    /// Gets the initial hashcode for the handler type in use.
    /// </summary>
    /// <returns>The initial hashcode value.</returns>
