    ...
    where TProgressHandler : class
{
    ...
    /// <summary>
    /// For IAsyncInfo instances that completed synchronously (at creation time) this field holds the result;
    ...
    /// <summary>
    /// Marks the current adapter as completed if the specified <see cref="System.Threading.Tasks.Task"/> instance is
    /// already completed, otherwise it registers a continuation to mark the current adapter as completed when it does.
    /// </summary>
    /// <param name="task">The input <see cref="System.Threading.Tasks.Task"/> instance.</param>
    private void InitializeCompletion(Task task)
    {
        // Fast-path: if the task is already completed, we can avoid the 'ContinueWith' overhead
        // and directly transition this adapter to the completed state, which is more efficient.
        if (task.IsCompleted)
        {
            TaskCompleted();
