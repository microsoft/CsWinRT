...
namespace UnitTest
{
    public class TestCSharp
    {
        ...
        public void TestVectorGetMany()
        {
            ...
            var boolSubset = TestObject.GetBooleanVectorSubset(bools, 1);
            Assert.True(bools.GetRange(1, 3).SequenceEqual(boolSubset));

            var blittableObjects = new List<BlittableStruct>()
            { 
                new BlittableStruct(1),
                new BlittableStruct(3),
                new BlittableStruct(5),
                new BlittableStruct(7),
                new BlittableStruct(9)
            };
            var blittableObjectsSubset = TestObject.GetBlittableVectorSubset(blittableObjects, 1);
            Assert.True(blittableObjects.GetRange(1, 3).SequenceEqual(blittableObjectsSubset));

            var nonBlittableObjects = new List<NonBlittableStringStruct>()
            {
                new NonBlittableStringStruct("one"),
                new NonBlittableStringStruct("two"),
                new NonBlittableStringStruct("three"),
                new NonBlittableStringStruct("four"),
                new NonBlittableStringStruct("five"),
                new NonBlittableStringStruct("six")
            };
            var nonBlittableObjectSubset = TestObject.GetNonBlittableVectorSubset(nonBlittableObjects, 1);
            ...
            var stringSubset = TestObject.GetStringVectorSubset(strings, 1);
            ...
            var classSubset = TestObject.GetClassVectorSubset(classObjects, 1);
            Assert.True(classObjects.GetRange(1, 3).SequenceEqual(classSubset));

            var objSubset = TestObject.GetObjectVectorSubset(classObjects, 1);
            Assert.True(classObjects.GetRange(1, 3).SequenceEqual(objSubset));

            var propertyObjectsSubset = TestObject.GetPropertiesVectorSubset(classObjects, 1);
            Assert.True(classObjects.GetRange(1, 3).SequenceEqual(propertyObjectsSubset));
        }
