// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

using WindowsRuntime.SourceGenerator.Tests.Helpers;

namespace WindowsRuntime.SourceGenerator.Tests;

/// <summary>
/// Tests for <see cref="AuthoringExportTypesGenerator"/> in the <c>CsWinRTImplementWinMDTypes</c> authoring
/// mode, where the Windows Runtime type being implemented is declared in an existing <c>.winmd</c>.
/// </summary>
/// <remarks>
/// The sources below declare the abstract bases inline, because that is what the user's compilation sees:
/// the projection writer emits them into the referenced projection (see <c>AbiImplementableClassFactory</c>).
/// </remarks>
[TestClass]
public class Test_AuthoringExportTypesGenerator
{
    [TestMethod]
    public void ImplementableClass_DispatchesOnWindowsRuntimeClassName()
    {
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                // The projected runtime class, from the '.winmd'
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget))]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }

                [WindowsRuntimeActivationFactory(typeof(MyWidget))]
                public sealed class MyWidgetFactory : global::ABI.Contoso.Widgets.WidgetActivationFactory
                {
                    public override object ActivateInstance() => new MyWidget();
                }
            }
            """;

        const string result = """
            // <auto-generated/>
            #pragma warning disable

            [assembly: global::WindowsRuntime.InteropServices.WindowsRuntimeComponentAssemblyExportsType(typeof(global::ABI.original.ManagedExports))]

            namespace ABI.original;

            using global::System;
            using global::System.CodeDom.Compiler;
            using global::System.ComponentModel;
            using global::System.Diagnostics;
            using global::System.Diagnostics.CodeAnalysis;
            using global::System.Runtime.CompilerServices;

            /// <summary>
            /// Contains the managed exports for activating types from the current project in an authoring scenario.
            /// </summary>
            [GeneratedCode("AuthoringExportTypesGenerator", <ASSEMBLY_VERSION>)]
            [DebuggerNonUserCode]
            [ExcludeFromCodeCoverage]
            [EditorBrowsable(EditorBrowsableState.Never)]
            public static unsafe class ManagedExports
            {
                private static readonly global::MyApp.MyWidgetFactory _activationFactory_Contoso_Widgets_Widget = new();

                /// <summary>
                /// Retrieves the activation factory from a DLL that contains activatable Windows Runtime classes.
                /// </summary>
                /// <param name="activatableClassId">The class identifier that is associated with an activatable runtime class.</param>
                /// <returns>The resulting pointer to the activation factory that corresponds with the class specified by <paramref name="activatableClassId"/>.</returns>
                public static void* GetActivationFactory(ReadOnlySpan<char> activatableClassId)
                {
                    if (activatableClassId.SequenceEqual("Contoso.Widgets.Widget".AsSpan()))
                    {
                        return (void*)global::ABI.Contoso.Widgets.WidgetActivationFactory.GetActivationFactoryUnsafe(_activationFactory_Contoso_Widgets_Widget);
                    }

                    return null;
                }

                /// <inheritdoc cref="GetActivationFactory(ReadOnlySpan{char})"/>
                /// <remarks>This overload uses reflection friendly types and is only meant to be used from managed runtime hosts.</remarks>
                public static nint GetActivationFactory(string activatableClassId)
                {
                    return (nint)GetActivationFactory(activatableClassId.AsSpan());
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifySources(source, ("ManagedExports.g.cs", result));
    }

    [TestMethod]
    public void StaticsOnlyClass_ResolvesNameFromFactoryBase()
    {
        // A statics-only runtime class has no instance type, so the runtime class name can only come from
        // the factory base. This is why discovery prefers it over the '[WindowsRuntimeActivationFactory]'
        // argument, which here deliberately points at the factory itself.
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public static class WidgetUtils;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.WidgetUtils))]
                public abstract class WidgetUtilsActivationFactory
                {
                    public abstract int Compute();

                    public static nint GetActivationFactoryUnsafe(WidgetUtilsActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                [WindowsRuntimeActivationFactory(typeof(MyWidgetUtilsFactory))]
                public sealed class MyWidgetUtilsFactory : global::ABI.Contoso.Widgets.WidgetUtilsActivationFactory
                {
                    public override int Compute() => 42;
                }
            }
            """;

        const string result = """
            // <auto-generated/>
            #pragma warning disable

            [assembly: global::WindowsRuntime.InteropServices.WindowsRuntimeComponentAssemblyExportsType(typeof(global::ABI.original.ManagedExports))]

            namespace ABI.original;

            using global::System;
            using global::System.CodeDom.Compiler;
            using global::System.ComponentModel;
            using global::System.Diagnostics;
            using global::System.Diagnostics.CodeAnalysis;
            using global::System.Runtime.CompilerServices;

            /// <summary>
            /// Contains the managed exports for activating types from the current project in an authoring scenario.
            /// </summary>
            [GeneratedCode("AuthoringExportTypesGenerator", <ASSEMBLY_VERSION>)]
            [DebuggerNonUserCode]
            [ExcludeFromCodeCoverage]
            [EditorBrowsable(EditorBrowsableState.Never)]
            public static unsafe class ManagedExports
            {
                private static readonly global::MyApp.MyWidgetUtilsFactory _activationFactory_Contoso_Widgets_WidgetUtils = new();

                /// <summary>
                /// Retrieves the activation factory from a DLL that contains activatable Windows Runtime classes.
                /// </summary>
                /// <param name="activatableClassId">The class identifier that is associated with an activatable runtime class.</param>
                /// <returns>The resulting pointer to the activation factory that corresponds with the class specified by <paramref name="activatableClassId"/>.</returns>
                public static void* GetActivationFactory(ReadOnlySpan<char> activatableClassId)
                {
                    if (activatableClassId.SequenceEqual("Contoso.Widgets.WidgetUtils".AsSpan()))
                    {
                        return (void*)global::ABI.Contoso.Widgets.WidgetUtilsActivationFactory.GetActivationFactoryUnsafe(_activationFactory_Contoso_Widgets_WidgetUtils);
                    }

                    return null;
                }

                /// <inheritdoc cref="GetActivationFactory(ReadOnlySpan{char})"/>
                /// <remarks>This overload uses reflection friendly types and is only meant to be used from managed runtime hosts.</remarks>
                public static nint GetActivationFactory(string activatableClassId)
                {
                    return (nint)GetActivationFactory(activatableClassId.AsSpan());
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifySources(source, ("ManagedExports.g.cs", result));
    }

    [TestMethod]
    public void FactoryWithoutGeneratedBase_IsNotRegistered()
    {
        // A factory that does not extend a generated base cannot supply 'GetActivationFactoryUnsafe', so it
        // is skipped. With no factories left, the managed exports are not emitted at all.
        const string source = """
            using WindowsRuntime.InteropServices;

            namespace MyApp
            {
                public sealed class MyWidget;

                [WindowsRuntimeActivationFactory(typeof(MyWidget))]
                public sealed class MyWidgetFactory : IActivationFactory
                {
                    public object ActivateInstance() => new MyWidget();
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifyNoSource(source, "ManagedExports.g.cs");
    }

    [TestMethod]
    public void ImplementableClass_IsNotMarkedAsComponentAssembly()    {
        // A project that only implements types declared in existing metadata produces no '.winmd' of its own,
        // so it must not be marked with '[WindowsRuntimeComponentAssembly]': that marker tells the tooling the
        // assembly's Windows Runtime types are projected into a generated 'WinRT.Component.dll'.
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget))]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget;

                [WindowsRuntimeActivationFactory(typeof(MyWidget))]
                public sealed class MyWidgetFactory : global::ABI.Contoso.Widgets.WidgetActivationFactory
                {
                    public override object ActivateInstance() => new MyWidget();
                }
            }
            """;

        Assert.IsFalse(GetManagedExports(source, isCsWinRTComponent: false).Contains("WindowsRuntimeComponentAssembly]"));

        // The same compilation built as an actual component does produce a '.winmd', so it keeps the marker
        Assert.IsTrue(GetManagedExports(source, isCsWinRTComponent: true).Contains("WindowsRuntimeComponentAssembly]"));

        // Either way it carries the exports-type marker, which is what puts it in the merged activation chain
        Assert.IsTrue(GetManagedExports(source, isCsWinRTComponent: false).Contains("WindowsRuntimeComponentAssemblyExportsType("));
        Assert.IsTrue(GetManagedExports(source, isCsWinRTComponent: true).Contains("WindowsRuntimeComponentAssemblyExportsType("));
    }

    [TestMethod]
    public void ImplementableClass_WithoutDeclaredFactory_GeneratesActivationFactory()
    {
        // A class whose only activation path is the parameterless 'ActivateInstance' needs no author input
        // beyond the implementation itself, so CsWinRT supplies the factory.
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget), HasDefaultActivationOnly = true)]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }
            }
            """;

        const string result = """
            // <auto-generated/>
            #pragma warning disable

            [assembly: global::WindowsRuntime.InteropServices.WindowsRuntimeComponentAssemblyExportsType(typeof(global::ABI.original.ManagedExports))]

            namespace ABI.original;

            using global::System;
            using global::System.CodeDom.Compiler;
            using global::System.ComponentModel;
            using global::System.Diagnostics;
            using global::System.Diagnostics.CodeAnalysis;
            using global::System.Runtime.CompilerServices;

            /// <summary>
            /// Contains the managed exports for activating types from the current project in an authoring scenario.
            /// </summary>
            [GeneratedCode("AuthoringExportTypesGenerator", <ASSEMBLY_VERSION>)]
            [DebuggerNonUserCode]
            [ExcludeFromCodeCoverage]
            [EditorBrowsable(EditorBrowsableState.Never)]
            public static unsafe class ManagedExports
            {
                private static readonly global::ABI.original.MyApp_MyWidgetActivationFactory _activationFactory_Contoso_Widgets_Widget = new();

                /// <summary>
                /// Retrieves the activation factory from a DLL that contains activatable Windows Runtime classes.
                /// </summary>
                /// <param name="activatableClassId">The class identifier that is associated with an activatable runtime class.</param>
                /// <returns>The resulting pointer to the activation factory that corresponds with the class specified by <paramref name="activatableClassId"/>.</returns>
                public static void* GetActivationFactory(ReadOnlySpan<char> activatableClassId)
                {
                    if (activatableClassId.SequenceEqual("Contoso.Widgets.Widget".AsSpan()))
                    {
                        return (void*)global::ABI.Contoso.Widgets.WidgetActivationFactory.GetActivationFactoryUnsafe(_activationFactory_Contoso_Widgets_Widget);
                    }

                    return null;
                }

                /// <inheritdoc cref="GetActivationFactory(ReadOnlySpan{char})"/>
                /// <remarks>This overload uses reflection friendly types and is only meant to be used from managed runtime hosts.</remarks>
                public static nint GetActivationFactory(string activatableClassId)
                {
                    return (nint)GetActivationFactory(activatableClassId.AsSpan());
                }
            }

            /// <summary>
            /// The activation factory for <see cref="global::MyApp.MyWidget"/>, which implements
            /// the <c>Contoso.Widgets.Widget</c> Windows Runtime class.
            /// </summary>
            [GeneratedCode("AuthoringExportTypesGenerator", <ASSEMBLY_VERSION>)]
            [DebuggerNonUserCode]
            [ExcludeFromCodeCoverage]
            [EditorBrowsable(EditorBrowsableState.Never)]
            internal sealed class MyApp_MyWidgetActivationFactory : global::ABI.Contoso.Widgets.WidgetActivationFactory
            {
                /// <inheritdoc/>
                public override object ActivateInstance()
                {
                    return new global::MyApp.MyWidget();
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifySources(source, ("ManagedExports.g.cs", result));
    }

    [TestMethod]
    public void ImplementableClass_WithDeclaredFactory_DoesNotGenerateActivationFactory()
    {
        // An author who declares the factory may be adding interop interfaces to its vtable that cannot be
        // inferred from here, so their factory is used rather than a generated one.
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget), HasDefaultActivationOnly = true)]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }

                [WindowsRuntimeActivationFactory(typeof(MyWidget))]
                public sealed class MyWidgetFactory : global::ABI.Contoso.Widgets.WidgetActivationFactory
                {
                    public override object ActivateInstance() => new MyWidget();
                }
            }
            """;

        string managedExports = GetManagedExports(source, isCsWinRTComponent: false);

        Assert.IsTrue(managedExports.Contains("global::MyApp.MyWidgetFactory _activationFactory_Contoso_Widgets_Widget"));
        Assert.IsFalse(managedExports.Contains("MyApp_MyWidgetActivationFactory"));
    }

    [TestMethod]
    public void ImplementableClass_WithoutDefaultActivation_DoesNotGenerateActivationFactory()
    {
        // Activating this class goes through factory methods, whose members only the author can implement
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget))]
                public abstract class WidgetActivationFactory
                {
                    protected abstract Widget CreateInstance(int value);

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifyNoSource(source, "ManagedExports.g.cs");
    }

    [TestMethod]
    public void ImplementableClass_WithoutParameterlessConstructor_DoesNotGenerateActivationFactory()
    {
        // The generated factory constructs the implementation, so it needs a reachable parameterless constructor
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget), HasDefaultActivationOnly = true)]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public MyWidget(int value) { }

                    public override void DoStuff() { }
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifyNoSource(source, "ManagedExports.g.cs");
    }

    [TestMethod]
    public void MultipleImplementationsOfSameClass_DoNotGenerateActivationFactory()
    {
        // Nothing here says which of the two to activate, so the author has to declare the factory
        const string source = """
            using WindowsRuntime;
            using WindowsRuntime.InteropServices;

            namespace Contoso.Widgets
            {
                public sealed class Widget;
            }

            namespace ABI.Contoso.Widgets
            {
                [WindowsRuntimeImplementableClass(typeof(global::Contoso.Widgets.Widget))]
                public abstract class Widget
                {
                    public abstract void DoStuff();
                }

                [WindowsRuntimeImplementableClassFactory(typeof(global::Contoso.Widgets.Widget), HasDefaultActivationOnly = true)]
                public abstract class WidgetActivationFactory : IActivationFactory
                {
                    public abstract object ActivateInstance();

                    public static nint GetActivationFactoryUnsafe(WidgetActivationFactory value) => throw null!;
                }
            }

            namespace MyApp
            {
                public sealed class MyWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }

                public sealed class MyOtherWidget : global::ABI.Contoso.Widgets.Widget
                {
                    public override void DoStuff() { }
                }
            }
            """;

        CSharpGeneratorTest<AuthoringExportTypesGenerator>.VerifyNoSource(source, "ManagedExports.g.cs");
    }

    /// <summary>
    /// Runs the generator and returns the generated managed exports source.
    /// </summary>    /// <param name="source">The input source to process.</param>
    /// <param name="isCsWinRTComponent">Whether to set the <c>"CsWinRTComponent"</c> MSBuild property to <see langword="true"/>.</param>
    /// <returns>The generated <c>ManagedExports.g.cs</c> source.</returns>
    private static string GetManagedExports(string source, bool isCsWinRTComponent)
    {
        return CSharpGeneratorTest<AuthoringExportTypesGenerator>.GetGeneratedSource(source, "ManagedExports.g.cs", isCsWinRTComponent: isCsWinRTComponent);
    }
}
