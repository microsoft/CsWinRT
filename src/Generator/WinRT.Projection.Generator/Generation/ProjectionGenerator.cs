...
internal static partial class ProjectionGenerator
{
    ...
    public static void Run([Argument] string responseFilePath, CancellationToken token)
    {
        ...
        {
            throw new UnhandledProjectionException("parsing", e);
        }
        ...
        {
            throw new UnhandledProjectionException("emit", e);
        }

        args.Token.ThrowIfCancellationRequested();

        // Notify the user that generation was successful
        ConsoleApp.Log($"Projection code generated -> {Path.Combine(args.GeneratedAssemblyDirectory, ProjectionDll)}");
    }
}
