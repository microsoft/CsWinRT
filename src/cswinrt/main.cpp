...
namespace cswinrt
{
    ...
    static constexpr option options[]
    {
        ...
        { "target", 0, 1, "<net10.0>", "Target TFM for projection (.NET 10 is the default)" },
...
Where <spec> is one or more of:
  ...
  10.0.12345.0[+]                    Specific version of Windows SDK [with extensions]
    ...
    void process_args(int const argc, char** argv)
    {
        ...
        if (!target.empty() && !starts_with(target, "net10.0"))
        {
            throw usage_exception();
        }
        else if (target.empty())
        {
            // Default to .NET 10 if no explicit target is set
            target = "net10.0";
        }
        settings.netstandard_compat = false;
