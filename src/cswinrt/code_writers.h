...
namespace cswinrt
{
    ...
    void write_typedef_name(writer& w, type_definition const& type, typedef_name_type const& nameType = typedef_name_type::Projected, bool forceWriteNamespace = false)
    {
        ...
        if (name_type_to_write == typedef_name_type::EventSource && typeNamespace == "System")
        {
            ...
            (name_type_to_write == typedef_name_type::EventSource && !w._in_abi_namespace) ||
        ...
        {
            w.write("global::");
            if (name_type_to_write == typedef_name_type::ABI || name_type_to_write == typedef_name_type::StaticAbiClass || name_type_to_write == typedef_name_type::EventSource)
            {
                w.write("ABI.");
            }
        ...
        else if (name_type_to_write == typedef_name_type::EventSource)
        {
            w.write("@%", typeName, "EventSource");
        }
        else
        {
            w.write("@", typeName);
    ...
    void write_interop_assembly_name(writer& w, TypeDef const& type)
    {
        ...
        if (auto proj = get_mapped_type(typeNamespace, type.TypeName()))
        {
            ...
            else if (starts_with(typeNamespace, "Windows"))
            {
                w.write("<#%Windows>");
                return;
            }
        ...
        if (starts_with(typeNamespace, "Windows"))
        {
            w.write("<#Windows>");
        }
        else
    ...
    void write_interop_dll_type_name(writer& w, type_semantics const& semantics, typedef_name_type const& nameType);
    void write_interop_dll_type_name_for_typedef(writer& w, type_definition const& type, typedef_name_type const& nameType, std::vector<type_semantics> generic_args = {})
    {
        if (nameType != typedef_name_type::Projected && nameType != typedef_name_type::InteropIID)
        {
            w.write("ABI.");
        }

        if (nameType == typedef_name_type::EventSource && type.TypeNamespace() == "Windows.Foundation")
        {
            w.write("WindowsRuntime.InteropServices.<#CsWinRT>EventHandlerEventSource'%", size(type.GenericParam()));
        }
        else
        {
            auto typeNamespace = std::string(type.TypeNamespace());
            auto typeName = std::string(type.TypeName());
            if (auto proj = get_mapped_type(typeNamespace, typeName))
            {
            ...
            // Arrays types are enclosed in <>
            if (nameType == typedef_name_type::ArrayMarshaller)
            {
                w.write("%.<%%", typeNamespace, bind<write_interop_assembly_name>(type), typeName);
            }
            else if (nameType == typedef_name_type::InteropIID)
            {
                w.write("%%", bind<write_interop_assembly_name>(type), typeName);
            }
            else if (nameType == typedef_name_type::Projected)
            {
                // Replace namespace seperator with _ within the generic due to that is what interop dll uses.
                std::replace(typeNamespace.begin(), typeNamespace.end(), '.', '-');
                w.write("%%-%", bind<write_interop_assembly_name>(type), typeNamespace, typeName);
            }
            else
            {
                w.write("%.%%", typeNamespace, bind<write_interop_assembly_name>(type), typeName);
        ...
        if (size(type.GenericParam()) != 0)
        {
            ...
            else
            {
                ...
                w.write("<%>",
                    ...
                    {
                        s();
                        write_interop_dll_type_name(w, w.get_generic_arg_scope(index++).first, typedef_name_type::Projected);
                    }, type.GenericParam()));                        
            }
        }
    ...
    void write_interop_dll_type_name(writer& w, type_semantics const& semantics, typedef_name_type const& nameType)
    {
        call(semantics,
            [&](object_type)
            {
                if (nameType == typedef_name_type::Projected)
                {
                    w.write("object");
                }
                else
                {
                    w.write("ABI.System.<object>");
                }
            },
            [&](guid_type)
            {
                ...
                {
                    w.write("ABI.System.<<#corlib>Guid>");
                }
            },
            [&](type_type)
            {
                ...
                {
                    w.write("ABI.System.<<#corlib>Type>");
                }
            },
            [&](type_definition const& type) { write_interop_dll_type_name_for_typedef(w, type, nameType); },
            [&](generic_type_instance const& type)
            {
                auto guard{ w.push_generic_args(type) };
            ...
            [&](fundamental_type const& type)
            {
                ...
                {
                    w.write("ABI.System.<%>", bind<write_fundamental_type>(type));
                }
            },
            [&](auto const&) {});

        if (nameType == typedef_name_type::StaticAbiClass)
        {
            w.write("Methods");
        }
        else if (nameType == typedef_name_type::Marshaller)
        {
            w.write("Marshaller");
        }
        else if (nameType == typedef_name_type::ArrayMarshaller)
        {
            w.write("ArrayMarshaller");
        }
    }
    ...
    static void write_iid_guid_interop_property_name(writer& w, TypeDef const& type)
    {
        w.write("IID_%", bind<write_interop_dll_type_name>(type, typedef_name_type::InteropIID));
    }
    ...
    {
        w.write("global::ABI.InterfaceIIDs.%", bind<write_iid_reference_guid_property_name>(type));
    }
    ...
    static void write_iid_guid(writer& w, TypeDef const& type)
    {
        ...
        {
            w.write("global::ABI.InterfaceIIDs.%", bind<write_iid_guid_property_name>(type));
        }
    }
            ...
            R"(public static ref readonly Guid %
{
    ...
    void write_abi_parameter(writer& w, method_signature::param_t const& param)
    {
        ...
        case param_category::fill_array:
            ...
            break;
        case param_category::receive_array:
            w.write(", uint* __%Size, %** %", param_name, bind<write_abi_type>(semantics), param_name);
            break;
        }
    }
    ...
    void write_abi_parameter_type_pointer(writer& w, method_signature::param_t const& param)
    {
        ...
        case param_category::fill_array:
            ...
            break;
        case param_category::receive_array:
            w.write(", uint*, %**", bind<write_abi_type>(semantics));
            break;
        }
    }
    ...
    void write_abi_return(writer& w, method_signature const& signature)
    {
        if (auto return_sig = signature.return_signature())
        {
            ...
            return_sig.Type().is_szarray() ?
                w.write(", uint* __%Size, %** %", signature.return_param_name(), bind<write_abi_type>(semantics), return_param) :
...
static extern % %([UnsafeAccessorType("%, WinRT.Interop")] object _, WindowsRuntimeObjectReference thisReference%%);
                    ...
                    bind<write_interop_dll_type_name>(parent, typedef_name_type::StaticAbiClass),
...
% %.%(%) => %.%(%);
    ...
    void write_class_method(
        writer& w,
        MethodDef const& method,
        TypeDef const& class_type, 
        bool is_overridable,
        bool is_protected,
        std::string_view platform_attribute,
        type_semantics static_method_semantics)
    {
        ...
        auto static_method_params = std::pair(static_method_semantics, method);
        if (!is_private)
        {
            write_method(w, signature, method.Name(), return_type, 
                w.write_temp("%", bind<write_objref_type_name>(static_method_params.first)), 
                access_spec, method_spec, platform_attribute, std::optional(static_method_params));
        }
...
%% %.%(%) => %;)",
                ...
                bind([&](writer& w)
                {
                    if (is_private)
                    {
                        w.write("%", bind<write_abi_static_method_call>(static_method_params.first, static_method_params.second,
                            w.write_temp("%", bind<write_objref_type_name>(static_method_params.first))));
                    }
                    else
                    {
...
static extern void %([UnsafeAccessorType("%, WinRT.Interop")] object _, WindowsRuntimeObjectReference thisReference, % value);
                ...
                prop_type,
                unsafe_accessor.value(),
                bind<write_interop_dll_type_name>(params_for_static_getter.value().first, typedef_name_type::StaticAbiClass),
                // set
                external_prop_name,
                unsafe_accessor.value(),
                bind<write_interop_dll_type_name>(params_for_static_getter.value().first, typedef_name_type::StaticAbiClass),
...
static extern % %_%([UnsafeAccessorType("%, WinRT.Interop")] object _, object thisObject, WindowsRuntimeObjectReference thisReference);
                ...
                external_event_name,
                bind<write_interop_dll_type_name>(parent_type, typedef_name_type::StaticAbiClass));
        }
...
private static WindowsRuntimeObjectReference %
{
    ...
    {
        var __% = field;
        if (__% != null && __%.IsInCurrentContext)
        {
            return __%;
        }
...
%}
                ...
                write_static_abi_method_with_raw_return_type(w, factory_type, method);
            }
        }
        else
        {
...
%}
                ...
                platform_attribute, 
                visibility,
                params_without_objects.empty() ? "" : "unsafe ",
                // construct name
                class_type.TypeName(),
            ...
            // Otherwise use the base class to handle activation.
            if (!params_without_objects.empty())
            {
...
IEnumerator IEnumerable.GetEnumerator() => %.GetEnumerator();
    ...
    void write_enumerable_members_using_static_abi_methods(writer& w, bool include_nongeneric, bool emit_explicit, std::string const& objref_name)
    {
        ...
        auto element_interop_type_name = w.write_temp("%", bind<write_interop_dll_type_name>(w.get_generic_arg_scope(0).first, typedef_name_type::Projected));

        w.write(R"(
[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "GetEnumerator")]
static extern IEnumerator<%> %GetEnumerator([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IEnumerable'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);
)",
    element,
...
static extern bool %MoveNext([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IEnumerator'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Current")]
static extern % %Current([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IEnumerator'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);
)",
// MoveNext
interop_method_name_prefix,
element_interop_type_name,
...
visibility, element, self, interop_method_name_prefix, objref_name);
    ...
    void write_readonlydictionary_members_using_static_abi_methods(writer& w, bool emit_explicit, std::string const& objref_name)
    {
        ...
        auto key_interop_type_name = w.write_temp("%", bind<write_interop_dll_type_name>(w.get_generic_arg_scope(0).first, typedef_name_type::Projected));
        auto value_interop_type_name = w.write_temp("%", bind<write_interop_dll_type_name>(w.get_generic_arg_scope(1).first, typedef_name_type::Projected));
...
static extern % %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IReadOnlyDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "ContainsKey")]
static extern bool %ContainsKey([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IReadOnlyDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "TryGetValue")]
static extern bool %TryGetValue([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IReadOnlyDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key, out % value);
)",
...
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, // Count
value, interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, // Item
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, // ContainsKey
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value // TryGetValue
);
...
[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Item")]
static extern % %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Item")]
static extern void %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key, % value);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Add")]
static extern void %Add([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key, % value);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "ContainsKey")]
static extern bool %ContainsKey([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Remove")]
static extern bool %Remove([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "TryGetValue")]
static extern bool %TryGetValue([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % key, out % value);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Add")]
static extern void %Add([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, KeyValuePair<%, %> item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Clear")]
static extern void %Clear([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Contains")]
static extern bool %Contains([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, KeyValuePair<%, %> item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "CopyTo")]
static extern void %CopyTo([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, WindowsRuntimeObjectReference enumObjRef, KeyValuePair<%, %>[] array, int arrayIndex);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Remove")]
static extern bool %Remove([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IDictionary'2<%|%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, KeyValuePair<%, %> item);
)",
key, interop_method_name_prefix, key_interop_type_name, value_interop_type_name, // Keys
value, interop_method_name_prefix, key_interop_type_name, value_interop_type_name, // Values
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, // Count
value, interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, // Item get
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // Item set
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // Add
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, // ContainsKey
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, // Remove
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // TryGetValue
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // Add
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, // Clear
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // Contains
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value, // CopyTo
interop_method_name_prefix, key_interop_type_name, value_interop_type_name, key, value // Remove
);
...
IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
    ...
    void write_readonlylist_members_using_static_abi_methods(writer& w, bool emit_explicit, std::string const& objref_name)
    {
        ...
        w.write(R"(
[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Count")]
static extern int %Count([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IReadOnlyList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Item")]
static extern % %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IReadOnlyList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, int index);
)",
...
element, enumerable_type, target);
    ...
    void write_list_members_using_static_abi_methods(writer& w, bool emit_explicit, std::string objref_name)
    {
        ...
        w.write(R"(
[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Count")]
static extern int %Count([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Item")]
static extern % %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, int index);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Item")]
static extern void %Item([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, int index, % value);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "IndexOf")]
static extern int %IndexOf([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Insert")]
static extern void %Insert([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, int index, % item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "RemoveAt")]
static extern void %RemoveAt([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, int index);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Add")]
static extern void %Add([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Clear")]
static extern void %Clear([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Contains")]
static extern bool %Contains([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % item);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "CopyTo")]
static extern void %CopyTo([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, %[] array, int arrayIndex);

[UnsafeAccessor(UnsafeAccessorKind.StaticMethod, Name = "Remove")]
static extern bool %Remove([UnsafeAccessorType("ABI.System.Collections.Generic.<#corlib>IList'1<%>Methods, WinRT.Interop")] object _, WindowsRuntimeObjectReference objRef, % item);
)",
interop_method_name_prefix, element_interop_type_name, // Count
element, interop_method_name_prefix, element_interop_type_name, // Item get
interop_method_name_prefix, element_interop_type_name, element,  // Item set
interop_method_name_prefix, element_interop_type_name, element, // IndexOf
interop_method_name_prefix, element_interop_type_name, element, // Insert
interop_method_name_prefix, element_interop_type_name, // RemoveAt
interop_method_name_prefix, element_interop_type_name, element, // Add
interop_method_name_prefix, element_interop_type_name, // Clear
interop_method_name_prefix, element_interop_type_name, element, // Contains
interop_method_name_prefix, element_interop_type_name, element, // CopyTo
interop_method_name_prefix, element_interop_type_name, element // Remove
);
...
visibility, self, objref_name);
    ...
    void write_class_members(writer& w, TypeDef const& type, bool is_interface_impl_type)
    {
...
return %.AsValue();
            ...
            for (auto&& prop : interface_type.PropertyList())
            {
                ...
                auto [prop_targets, inserted] = properties.try_emplace(property_name,
                    ...
                    is_private,
                    getter ? std::optional(std::pair(semantics_for_abi_call, prop)) : std::nullopt,
                    setter ? std::optional(std::pair(semantics_for_abi_call, prop)) : std::nullopt
                );
                if (!inserted)
                {
                    ...
                    if (getter)
                    {
                        ...
                        getter_platform = platform_attribute;
                        getter_prop = std::optional(std::pair(semantics_for_abi_call, prop));
                    }
                    if (setter)
                    {
                        XLANG_ASSERT(setter_target.empty());
                        ...
                        setter_platform = platform_attribute;
                        setter_prop = std::optional(std::pair(semantics_for_abi_call, prop));
                    }
...
bind<write_interop_dll_type_name>(semantics, typedef_name_type::EventSource)
    ...
    void write_event_source_ctor_call(writer& w, Event const& evt, uint32_t const& abi_methods_start_index = 6)
    {
        auto semantics = get_type_semantics(evt.EventType());
        auto [add, _] = get_event_methods(evt);
        auto index = get_vmethod_index(add.Parent(), add) + abi_methods_start_index;
        if (std::holds_alternative<generic_type_instance>(semantics))
        {
            w.write("Unsafe.As<%>(ctor(thisReference, %))",
                bind<write_type_name>(semantics, typedef_name_type::EventSource, false),
                index);
        }
        else
        {
            w.write("new %(thisReference, %)",
                bind<write_type_name>(semantics, typedef_name_type::EventSource, false),
                index);
        }
    }
...
event % %;)",
    ...
    struct abi_marshaler
    {
        ...
        std::string get_inline_array_type() const
        {
            if (local_type == "void*")
            {
                return "nint";
...
static extern %[] ConvertToManaged_%([UnsafeAccessorType("%, WinRT.Interop")] object _, uint length, %* data);
...
static extern void Dispose_%([UnsafeAccessorType("%, WinRT.Interop")] object _, uint length, %* data);
...
static extern void Free_%([UnsafeAccessorType("%, WinRT.Interop")] object _, uint length, %* data);
...
static extern void ConvertToUnmanaged_%([UnsafeAccessorType("%, WinRT.Interop")] object _, ReadOnlySpan<%> span, out uint length, out %* data);
...
static extern void CopyToUnmanagedUnsafe_%([UnsafeAccessorType("ABI.System.<string>ArrayMarshaller, WinRT.Interop")] object _, ReadOnlySpan<string> value, uint destinationSize, void* destination, uint headersSize, void* headers);
...
static extern void CopyToUnmanaged_%([UnsafeAccessorType("%, WinRT.Interop")] object _, ReadOnlySpan<%> span, uint length, %* data);
...
static extern void CopyToManaged_%([UnsafeAccessorType("%, WinRT.Interop")] object _, uint length, %* data, Span<%> span);
...
Dispose_%(null, (uint) __%_span.Length, %_%);
                            ...
                            local_type,
                            // If it is already a pointer type,
                            // we can pass the pointer from the fixed expression.
                            // And this allows us to handle that the span is of type nint.
                            local_type == "void*" ? "" : "*",
                            param_name,
                            param_name,
                            param_name,
...
CopyToUnmanaged_%(null, __%, __%Size, (%*)%);
    ...
    auto get_managed_marshalers(writer& w, method_signature const& signature, bool /*is_generic*/, bool is_generic_instantiation_class)
    {
            ...
            [&](writer& w, type_semantics const& semantics, managed_marshaler& m)
        {
            ...
            if (m.is_array())
            {
                ...
                m.interop_dll_type = w.write_temp("%", bind<write_interop_dll_type_name>(semantics, typedef_name_type::ArrayMarshaller));
                m.abi_type = w.write_temp("%", bind<write_abi_type>(semantics));
            }
        };
...
%
    ...
    void write_event_abi_invoke(writer& w, Event const& evt)
    {
        ...
        auto semantics = get_type_semantics(evt.EventType());
        auto generic_event_type = std::holds_alternative<generic_type_instance>(semantics);
...
private static ConditionalWeakTable<%, EventRegistrationTokenTable<%>> _%
{
    ...
    {
        [MethodImpl(MethodImplOptions.NoInlining)]
        static ConditionalWeakTable<%, EventRegistrationTokenTable<%>> MakeTable()
        {
            _ = global::System.Threading.Interlocked.CompareExchange(ref field, [], null);
...
try
{
var __this = ComInterfaceDispatch.GetInstance<%>((ComInterfaceDispatch*)thisPtr);
%
*% = _%.GetOrCreateValue(__this).AddEventHandler(__handler);
__this.% += __handler;
return 0;
}
catch (Exception __exception__)
...
var __handler = ConvertToManaged(null, %);
                        ...
                        bind<write_type_name>(semantics, typedef_name_type::Projected, false),
                        bind<write_interop_dll_type_name>(semantics, typedef_name_type::Marshaller),
...
{
var __this = ComInterfaceDispatch.GetInstance<%>((ComInterfaceDispatch*)thisPtr);
if(__this is not null && _%.TryGetValue(__this, out var __table) && __table.RemoveEventHandler(%, out var __handler))
{
__this.% -= __handler;
}
return 0;
...
private IObjectReference % => __% ?? Make__%();
            ...
            bind<write_class_members>(iface, true));
    }
...
return value.GetDefaultInterface();
        ...
        projected_type_name,
        projected_type_name,
        sealed ? "WindowsRuntimeObjectMarshaller" : "WindowsRuntimeUnsealedObjectMarshaller",
        type.TypeName());
    }
