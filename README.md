...
**Note:** When building the first time, the build clones the testwinrt project. The build depends on this project, but msbuild doesn't consistently pick up the change when the repo is first cloned. You may need to build a second time if this is your first build to address the timing issue.
