...
- For each interface the composable runtime class declares, the inner object allocates a private copy of the normal CCW vtable, and replaces only its six `IInspectable` (and therefore `IUnknown`) entries with delegating ones (`WindowsRuntimeAggregationIInspectableImpl`). Every other entry is left untouched, so interface methods keep running the exact same CCW stubs, and still receive a real `ComInterfaceDispatch` pointer.
