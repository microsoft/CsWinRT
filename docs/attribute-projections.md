...
| `[Windows.Foundation.Metadata.CreateFromString(MethodName = "...")]` | itself, including the named `MethodName` field on runtime classes and structs, so the XAML compiler can discover their string conversion methods | reference only |
