...
The automatic provider returns no property or indexer descriptors and uses the object's `ToString()` for its string representation. Property binding still requires an explicit provider, such as one generated by `[GeneratedCustomPropertyProvider]`. Existing providers, including inherited implementations and their property lists, take precedence regardless of their base type. Types that do not derive from `FrameworkElement`, including other `DependencyObject` and `UIElement` subclasses, do not acquire this fallback.
...
This maps to the `CSWINRT_ENABLE_XAML_CUSTOM_PROPERTY_PROVIDER` runtime feature switch, which defaults to `true` and is recognized by ILLink and Native AOT. When disabled, the fallback interface is not exposed and its implementation can be trimmed. Explicit and source-generated providers are unaffected. Disabling the bridge can prevent XAML from recognizing unannotated custom controls when applying typed styles or templates.
