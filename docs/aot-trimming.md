...
| `CsWinRTEnableXamlCustomPropertyProvider` | `true` | Automatically exposes a type-only `ICustomPropertyProvider` on managed XAML-derived objects that do not already provide one. |
...
Managed types derived from projected WinUI or UWP XAML classes automatically expose a **type-only** native `ICustomPropertyProvider` implementation. This lets XAML identify the actual managed type when applying typed styles and templates, without requiring an attribute or `partial` modifier on custom controls. The bridge does not change `IInspectable.GetRuntimeClassName`, which can still report an implemented interface for non-authored types.

The automatic provider also covers `DependencyObject` subclasses without overridable interfaces. It returns no property or indexer descriptors and uses the object's `ToString()` for its string representation. Property binding still requires an explicit provider, such as one generated by `[GeneratedCustomPropertyProvider]`. Existing providers, including inherited implementations and their property lists, take precedence. Other managed CCWs do not acquire this fallback.
