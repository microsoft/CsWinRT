...
| `CsWinRTEnableXamlCustomPropertyProvider` | `true` | Automatically exposes a type-only `ICustomPropertyProvider` on managed `FrameworkElement`-derived objects that do not already provide one. |
...
Managed types that directly or indirectly derive from the projected `Microsoft.UI.Xaml.FrameworkElement` or `Windows.UI.Xaml.FrameworkElement` automatically expose a **type-only** native `ICustomPropertyProvider` implementation. `FrameworkElement` exposes the `Style` property, and the bridge lets XAML identify the actual managed type when applying typed styles and templates, without requiring an attribute or `partial` modifier on custom controls. The bridge does not change `IInspectable.GetRuntimeClassName`, which can still report an implemented interface for non-authored types.
...
Existing providers, including inherited implementations and their property lists, take precedence regardless of their base type. Their lookup behavior is unchanged: for example, a source-generated provider returns null for a property it does not expose. Types that do not derive from `FrameworkElement`, including other `DependencyObject` and `UIElement` subclasses, do not acquire the automatic fallback.

To opt out of only the automatic type bridge, set:

```xml
<PropertyGroup>
  <CsWinRTEnableXamlCustomPropertyProvider>false</CsWinRTEnableXamlCustomPropertyProvider>
</PropertyGroup>
```
...
The matching `CSWINRT_ENABLE_XAML_CUSTOM_PROPERTY_PROVIDER` runtime configuration definition is retained for consistency with other feature switches, but no runtime code currently reads it. Changing `AppContext` configuration alone does not change the generated entries. Disabling the bridge can prevent XAML from recognizing unannotated custom controls when applying typed styles or templates.
