...
If your app or library has non-WinRT types that implement C#/WinRT projected interfaces, extend C#/WinRT projected classes, or implement WinRT mapped built-in .NET interfaces and are passed across the ABI to other WinRT functions, the type needs to be marked `partial` and if using `OptIn` mode also needs to be have the `WinRT.GeneratedWinRTExposedType` attribute. This enables the source generator distributed with C#/WinRT to add an attribute to the type with the WinRT vtable for it such that it can be looked up later in a way that is both trimming and AOT compatible.

This applies to all type kinds that can be passed across the ABI, so classes, structs, records and record structs. Value types are included because they can implement WinRT interfaces too (for instance, `System.Collections.Immutable.ImmutableArray<T>` implements `IReadOnlyList<T>` and `IList`) and get boxed when passed across the ABI. The only exception is `ref struct` types, which can never be boxed and are therefore always skipped. When the generator adds the partial definition for a type, it uses the same keyword as the original declaration, so a `record struct` stays a `record struct`.
