...
| `--analyze-net-standard-assemblies` | `bool` | Default `true`; whether .NET Standard assemblies participate in discovery, including explicit opt-ins (from `CsWinRTAnalyzeNetStandardAssemblies`) |
| `--marshalling-enabled-assembly-names` | `string[]` | Assembly names (`.dll`/directory ignored) analyzed regardless of mode, subject to framework exclusions (from the `CsWinRTMarshallingEnabledAssembly` item) |
...
**Framework exclusions** (`ModuleDefinition.TargetsNetFramework` and `TargetsNetStandard`): .NET Framework assemblies are never analyzed. .NET Standard assemblies participate by default through the invocation's resolution-aware, version-agnostic `SignatureComparer`. Setting `CsWinRTAnalyzeNetStandardAssemblies=false` skips them before both marshalling-mode filtering and explicit opt-ins. The property is included in `_ComputeRunCsWinRTGeneratorCache`, forwarded by `RunCsWinRTInteropGenerator`, and preserved in saved and replayed debug repros.

**Opt-in assemblies** (`--marshalling-enabled-assembly-names`, from the `CsWinRTMarshallingEnabledAssembly` MSBuild item): assemblies listed here are analyzed (matched by simple name, ignoring `.dll`/directory) regardless of mode, but do not override the framework exclusions above. This lets users fine-tune size (e.g. `strict` mode + a few opted-in assemblies). `ValidateMarshallingEnabledAssemblies` emits three diagnostics: `CSWINRTINTEROPGEN0098` (warning: entry doesn't match any referenced assembly), `CSWINRTINTEROPGEN0099` (message: entry already targets Windows, redundant), and `CSWINRTINTEROPGEN0100` (message: mode is `all`, all entries redundant).
...
- Must be a projected Windows Runtime type. A type is recognized as projected in any of three ways: it carries the per-type `[WindowsRuntimeType]` marker (implementation projections and types in `WinRT.Runtime.dll`); it is an exported type from an authored component assembly (`IsComponentWindowsRuntimeType`); or it is defined in a reference projection assembly marked `[WindowsRuntimeReferenceAssembly]` (`IsReferenceProjectionWindowsRuntimeType`). The latter two do **not** carry the per-type `[WindowsRuntimeType]` marker. For authored components, the assembly marker and public top-level visibility are necessary but not sufficient: the type must also appear in the `ABI.WindowsRuntimeMetadataTypes` lookup in `WinRT.Component.dll`. The projection writer records the original authored types from the component's WinMD there, alongside generated projection types. Internal, nested, and deliberately non-exported public helpers therefore retain ordinary managed-type behavior, including when used in generic delegates and collections. Reference projections shipped in NuGet packages are still recognized by their reference-assembly marker.
